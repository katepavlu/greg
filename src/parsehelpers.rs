...
/// 
/// uses sel_token to read an immediate. Forwards errors
pub fn get_identifier_or_imm(ret: Option<LexerResult>)
...
mod tests {
    ...
    fn reading() {
        ...
        assert_eq!(read_token(lexer.next()).unwrap().1, Token::Instruction(Instr::Add));
        assert_eq!(read_token(lexer.next()).unwrap().1, Token::Register(0));
        assert_eq!(read_token(lexer.next()).unwrap().1, Token::Identifier("loop_start".to_string()));
    ...
    fn selecting() {
        ...
        assert_eq!(sel_token(lexer.next(), Token::Comma).unwrap().1, Token::Comma);
        assert_eq!(sel_token(lexer.next(), Token::Colon).unwrap().1, Token::Colon);
        assert_eq!(sel_token(lexer.next(), Token::Colon), Err(ParserError::Incomplete(Loc{row:0, col:4})));
        assert_eq!(sel_token(lexer.next(), Token::Colon), Err(ParserError::InvalidToken(Loc{row:0, col:8})));
    }
    ...
    fn register() {
        ...
        assert_eq!(get_register(lexer.next()), Err(ParserError::Incomplete(Loc{row:1, col:1})));
        assert_eq!(get_register(lexer.next()), Err(ParserError::InvalidToken(Loc{row:1, col:4})));
    }
    ...
    fn immediate() {
        ...
        assert_eq!(get_register(lexer.next()), Err(ParserError::InvalidToken(Loc{row:0, col:8})));
    }
    ...
    fn identifier_or_imm() {
        ...
        assert_eq!(get_identifier_or_imm(lexer.next()).unwrap(), ("".to_owned(), -12) );
        assert_eq!(get_identifier_or_imm(lexer.next()).unwrap(), ("greg".to_owned(), 0) );
        assert_eq!(get_register(lexer.next()), Err(ParserError::InvalidToken(Loc{row:0, col:9})));
    }
